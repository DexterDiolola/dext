macStats.factory('myService',['$http', function($http){
	return{
		...
		//CUSTOM FUNCTIONS
		packageSummary: function(mac, nthDay){
			return $http({
				method: 'GET',
				url: '/api/package-summary?mac=' + mac + '&nthDay=' + nthDay 
			});
		},
...
['$scope', '$http', 'myService', function($scope, $http, myService){
	...
	myService.getActiveMacs('countActivePM', 'getMac', '').then(function(response){
		...
		for(var x=0; x<$scope.a.length; x++){
			for(var y=0; y<30; y++){
				myService.packageSummary($scope.a[x], y).then(function(response){
...
// use $location.hash() to get current url hash eg.(/macs?mac=1011200107&foo=bar) returns foo=bar as object
	...
	for(var y=0; y<30; y++){
		myService.packageSummary($scope.macParam, y).then(function(response){
